...
class HAN():
    ...
    def doc2vec(self, sent_vec):
        ...
        with tf.name_scope("doc2vec"):
            ...
            doc_vec = self.AttentionLayer_sent(doc_encoded, name='sent_attention')
            return doc_vec
    ...
    def AttentionLayer(self, inputs, name):
        ...
        with tf.variable_scope(name):
            ...
            reinputs = tf.multiply(inputs, alpha)
            atten_output_tp = tf.reduce_sum(reinputs, axis=1)
            h_ = layers.fully_connected(reinputs, self.hidden_size * 2, activation_fn=tf.nn.tanh)
            alpha_ = tf.nn.softmax(tf.reduce_sum(tf.multiply(h_, u_context), axis=2, keep_dims=True), dim=1)
            atten_output = tf.reduce_sum(tf.multiply(reinputs, alpha_), axis=1)
            atten_output = tf.add(atten_output, atten_output_tp)
            return atten_output

    def AttentionLayer_sent(self, inputs,name):
        # inputs是GRU的输出，size是[batch_size, max_time, encoder_size(hidden_size *  2)]
        with tf.variable_scope(name):
            # u_context是上下文的重要性向量，用于区分不同单词/句子对于句子/文档的重要程度,
            # 因为使用双向GRU，所以其长度为2×hidden_szie
            ...
            h = layers.fully_connected(inputs, self.hidden_size * 2, activation_fn=tf.nn.tanh)
            # shape为[batch_size, max_time, 1]
            s = tf.multiply(h, u_context)
            ss = tf.reduce_sum(s, axis=2, keep_dims=True)
            alpha = tf.nn.softmax(ss, dim=1)
            # reduce_sum之前shape为[batch_size, max_time, hidden_size*2]，之后shape为[batch_size, hidden_size*2]
