...
tf.flags.DEFINE_integer("vocab_size", 15541, "vocabulary size")
tf.flags.DEFINE_integer("num_classes", 5, "number of classes")
tf.flags.DEFINE_integer("embedding_size", 150, "Dimensionality of character embedding (default: 150)")
...
tf.flags.DEFINE_integer("checkpoint_every", 70, "Save model after this many steps (default: 70)")
...
with tf.Session() as sess:
    ...
    #hard_alpha = tf.Variable([64*50, 50])
    #soft_alpha = tf.Variable([64, 50])
    ...
    def train_step(x_batch, y_batch):
        feed_dict = {
            han.input_x: x_batch,
            han.input_y: y_batch,
            han.max_sentence_num: 50,
            han.max_sentence_length: 50,
            han.batch_size: 44
        }
    ...
    def dev_step(x_batch, y_batch, writer=None):
        ...
        step, summaries, cost, accuracy , hard_alpha, soft_alpha , out = sess.run([global_step, dev_summary_op, loss, acc,han.hard_alpha, han.soft_alpha, han.out], feed_dict)
        pickle.dump(hard_alpha.tolist(), open('word_attn', 'wb'))#jzd
        pickle.dump(soft_alpha.tolist(), open('sent_attn', 'wb'))#jzd
        pickle.dump(out.tolist(), open('out', 'wb'))#jzd
    ...
    for epoch in range(FLAGS.num_epochs):
        ...
        for i in range(0, 4460, FLAGS.batch_size):
