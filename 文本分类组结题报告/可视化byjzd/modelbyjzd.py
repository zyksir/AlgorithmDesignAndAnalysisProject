...
class HAN():
    ...
    def sent2vec(self, word_embedded):
        ...
        with tf.name_scope("sent2vec"):#定义命名域
            ...
            # 对每个句子进行attention处理
            sent_vec = self.AttentionLayerHard(word_encoded, name='word_attention')#赋权
            return sent_vec
    ...
    def doc2vec(self, sent_vec):
        ...
        with tf.name_scope("doc2vec"):
            ...
            doc_vec = self.AttentionLayerSoft(doc_encoded, name='sent_attention')
            return doc_vec
    ...
    def classifer(self, doc_vec):
        with tf.name_scope('doc_classification'):
            ...
            writer = tf.summary.FileWriter('./logs', tf.get_default_graph())#jzd
            writer.close()#jzd
            return out
    ...
    def AttentionLayerSoft(self, inputs, name):
